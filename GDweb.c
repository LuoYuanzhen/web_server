...
Heap* initHeap(int num_item)
{
    Heap* h=(Heap*)malloc(sizeof(Heap));
    pthread_mutex_init(&h->mutex,NULL);
    h->item=(Item*)malloc(num_item* sizeof(Item));
...
void adjustUpHeap(Heap* heap,int index)
{
    ...
    while(child>0)
    {
        if(heap->item[child].k<heap->item[parent].k)
        {
...
void adjustDownHeap(Heap* heap,hashpair* pair)
{
    ...
    {
        if(((child+1)<heap->size)&&(heap->item[child+1].k<heap->item[child].k))
            child=child+1;
        if(heap->item[parent].k>heap->item[child].k)
        {
            Item item=heap->item[child];
            heap->item[child]=heap->item[parent];
            heap->item[parent]=item;
  ...
  if(( file_fd = open(&(buffer[5]),O_RDONLY)) == -1) {  /* open the file for reading */
    logger(NOTFOUND, "failed to open file",&(buffer[5]),fd);
      ...
      if (bucketcon == NULL)//没读到
      {
          ...
          char *newbuffer = (char *) malloc(sizeof(char) * len );
          if ((ret = read(file_fd, newbuffer, len)) > 0) {
      ...
      {
          int hash=hashString(filekey)%table->num_bucket;
          hash=hash>0?hash:(-hash);
...
int main(int argc, char **argv)
{
  ...
  send_msg_pool=initThreadPool(16);
